...
static const std::string MQTT_CONFIG_CLIENT_USERMANE = "client_username"; // Key to store MQTT username (string)
static const std::string MQTT_CONFIG_CLIENT_PASSWORD = "client_password"; // Key to store MQTT password (string)
static const std::string MQTT_CONFIG_TLS_ENABLE = "tls_enabled";          // Key to store MQTT TLS activation (uint8_t), 0 if TLS disabled
static const std::string MQTT_CONFIG_BROKER_CERT = "broker_cert";         // Key to store MQTT broker cert (string)
static const std::string MQTT_CONFIG_DISCOVERY_ID = "disc_id";            // Key to store MQTT discovery ID (string)
static const std::string MQTT_CONFIG_TOPIC_PREFIX = "topic_prefix";       // Key to store MQTT topic prefix (string)
static const std::string MQTT_CONFIG_DISCOVERY_PREFIX = "disc_prefix";    // Key to store MQTT discovery prefix (string)
...
namespace Config
{
    void MqttConfig::DeleteMqttConfig()
    {
        ...
        NvsHelpers::DeleteValue(MQTT_CONFIG_NAMESPACE, MQTT_CONFIG_DISCOVERY_ID);
    ...
    const std::string MqttConfig::GetDiscoveryIdName()
    {
        std::string id = CONFIG_MQTT_DISCOVERY_ID_NAME;
        NvsHelpers::GetString(MQTT_CONFIG_NAMESPACE, MQTT_CONFIG_DISCOVERY_ID, id);
        return id;
    }
    esp_err_t MqttConfig::SetDiscoveryIdName(const std::string &id)
    {
        return NvsHelpers::SetString(MQTT_CONFIG_NAMESPACE, MQTT_CONFIG_DISCOVERY_ID, id);
    }
