...
static int do_config_mqtt_cmd(int argc, char **argv)
{
    ...
    if (config_mqtt_args.read->count > 0)
    {
        ...
        ESP_LOGI(TAG, "Discovery ID: %s", MqttConfig::GetDiscoveryIdName().c_str());
    ...
    else
    {
        ...
        if (config_mqtt_args.discovery_id->count > 0)
        {
            err = MqttConfig::SetDiscoveryIdName(config_mqtt_args.discovery_id->sval[0]);
            if (err != ESP_OK)
            {
                ESP_LOGE(TAG, "Failed to set discovery ID to configuration storage! (%d)", err);
            }
            else
            {
                ESP_LOGI(TAG, "Discovery ID set to configuration storage: %s", MqttConfig::GetDiscoveryIdName().c_str());
            }
        }
...
void register_config_mqtt(void)
{
    ...
    config_mqtt_args.discovery_id = arg_str0(NULL, "disc_id", "<discovery_id>", "ID and Name to use in discovery message");
